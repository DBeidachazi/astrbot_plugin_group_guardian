...
- 新增独立的 `encoded_content.py` 受限解码模块，支持 Base16/Hex、Base32、Base58、Base62、
  Base64、Base64URL、Base85/ASCII85，并支持最多两层嵌套编码。
- 解码证据统一进入现有本地规则、同发送者上下文和 LLM 审核，不另建一套处罚逻辑；原文、OCR、
  二维码及合并转发提取出的编码内容均可参与审核。
- 支持跨多条消息重组编码，修复聚合正文被统一转小写、破坏大小写敏感 Base 编码的问题；仅去重签名
  做小写归一化，审核正文保留原始大小写。
- 自动识别仅接受严格 UTF-8 可读文本；GB18030 只在用户显式写出 `baseXX:` 前缀时启用，降低随机
  令牌、哈希和普通标识符误判风险。
- 设置候选数、递归深度、单令牌、单结果、总输入和总输出硬上限；“可以解码”只触发语义复核，
  LLM 故障时不会仅因编码证据而 fail-closed 误封。
- 新增默认开启、可按群覆盖的 `base_decode_enabled`，并在 WebUI「审核」页提供“Base 解码审核”开关。

### 新功能：AI 误判复盘（Issue #67）

- WebUI「违规记录」支持把已处罚日志人工标记为“误判”或“确认违规”；反馈会保存当时的消息、处罚和
  原因快照，原审核日志后续删除也不影响复盘证据。
- 可手动立即复盘，也可默认关闭地定期复盘；手动模式一条误判即可运行，自动模式达到可配置样本门槛
  才调用 LLM，并可使用确认违规样本作对照。
- LLM 只生成待管理员审阅的修正规则候选，不会静默修改生产审核提示词；候选支持明确应用、拒绝、
  回滚和完整审计记录。
- 已应用修正规则同时追加到内置审核标准和用户自定义审核标准，框架的 JSON 输出约束始终保留。
- 增加旧值冲突保护和配置持久化校验：配置被其他操作修改时禁止覆盖；保存失败时不改变候选状态，
  防止 WebUI 状态与重启后实际规则不一致。
- LLM 复盘期间若管理员清除反馈或改判为确认违规，事务内的二次状态校验会放弃过期候选，避免旧结论
  污染修正规则。
- 误判样本按不可信数据封装并限制长度，样本中的角色指令或输出要求不会成为复盘指令。
- 复盘面板和全部接口只使用 AstrBot Dashboard 的 `register_web_api` 内置路由，未新增独立 Web 服务、
  监听端口或 `0.0.0.0` 地址。

### 修复与性能优化

- 定时解禁现在校验 OneBot `status/retcode`：成功后才删除计划；失败保留任务并按 30 秒起步、最多
  1 小时的指数退避重试。旧版 SQLite 表会自动补齐重试次数、下次重试时间和最后错误字段。
- 图片下载在 aiohttp 实际建连解析阶段再次校验全部 DNS 结果，只允许公网地址，关闭“预检查后 DNS
  重绑定到内网”的 SSRF 时间窗口；混合公网/私网解析同样拒绝。
- WebUI 规则统计查询移入工作线程并并发执行，避免大词库计数阻塞事件循环。
- WebUI 审核日志、反馈与总数查询移入线程池；通用配置保存失败会恢复本次内存修改并返回错误，不再
  显示“保存成功”。复盘开关或间隔热更新会唤醒调度循环并按新配置重新计时。
- 词库加载从“每个分类一次查询”的 N+1 模式改为两次批量查询；seed 词库连接在查询异常时也会关闭。
- 修复中文正文紧贴 `base16:` / `base85:` 等显式前缀时受 Unicode 单词边界影响而漏解码的问题。
- 修复 SQLite 日志曾短暂写入失败时，WebUI 仍能看到内存日志却无法标记误判的问题；标记前会先补写
  持久层，再保存反馈快照。
- 清理两个全仓无调用的 Web 辅助方法、未读取的组合状态变量，以及多处无用途导入和局部变量；保留
  AstrBot 装饰器动态入口与视觉依赖探测代码，不改变现有功能行为。
- 补充 Base 多编码/双层/拆分/关闭开关/资源边界、真实 SQLite 旧表迁移与重试、OneBot 协议失败、
  DNS 私网与混合解析、误判复盘状态机、提示词拼接和内置 WebUI 路由等回归测试。
...
- 正式发布全量上下文审核与多图并发修复，版本号提升后 AstrBot 可明确识别并提示本次更新。
- 超长正文及多图 OCR/二维码证据按重叠分片完整送审，修复中间证据被摘要截断的问题。
- 图片、LLM 和群历史采用受控并发及明确超时边界，避免高峰漏审、无限积压或同一用户后续消息永久等待。

## v2.7.5 - 2026-07-28

### 增强：全量 AI 审核与连续上下文

- 新增可按群覆盖的 `llm_moderation_always`（默认关闭）：开启后，未命中本地规则的普通消息也会进入 LLM 审核；图片消息会自动尝试 OCR、表情包识别和二维码解析，再将识别文本与正文一并审核。
- 建立独立于防刷屏队列的审核上下文，按消息序号、时间和到达顺序整理群聊历史及同一发送者近期分段；提示词明确要求把逐字、逐词拆分内容作为连续整体判断。
- 修复组合检测按用户整体冷却导致新片段被跳过的问题，改为按具体候选签名去重；处罚、放行和处理中状态形成明确边界，既避免同一事件重复送审，也不会阻断后续新组合。
- 普通 AI 模式下，同一发送者形成逐字、联系方式或引流形态的多条组合后，即使本地规则零命中也会进入语义审核；已启用 OCR/二维码识别的截图文本同样强制进入 LLM，覆盖 `日抛plus` 后另发 `/xxxxxx` 或把引流内容放进截图的场景。
- 无 `message_id` 的事件改用实例级单调键并缓存到事件，避免 Python 对象地址复用造成不同消息互相覆盖；本地上下文和候选签名均增加容量上限及节流清理。

### 优化：多图吞吐与高并发边界

- 多图 OCR 和二维码解析改为全部图片稳定去重后受控并发，单图失败隔离并保持原图顺序；OCR 与二维码两个分支并行执行，远程合并转发图片也进入同一识图管线。
- 新增 `llm_max_concurrency`（默认 12，范围 1-32），统一限制消息、OCR、入群和名片 LLM 调用；OCR 另设并发配额，防止多图任务占满正文审核通道。
- 群历史请求增加独立并发限制、同群短时缓存和进行中请求合并；图片下载增加全局 I/O 限流并复用插件级 HTTP 会话，插件卸载时主动关闭资源。
- 纯图片消息会在识图前登记顺序占位，后续同用户消息等待更早图片完成；OCR/二维码证据缓存 15 分钟并可被群历史复用，避免高并发乱序和重复视觉调用。
- LLM 与 OCR 的正常峰值会等待并发许可，长期过载则按明确超时降级，避免事件任务无限积压；普通 AI 模式只对逐字片段、联系方式和引流形态的零命中组合做语义复核，降低正常连续发言的调用量。
- 超长全量审核内容按重叠分片完整送审，多图 OCR/二维码结果按图片公平保留证据；图片处理使用固定工作协程，并为 DNS、二维码线程和单条消息图片总时长补齐边界，避免中间图片漏审或后续消息永久等待。
- 图片下载改为循环读取分块响应直至 EOF，并继续执行大小、超时、重定向和地址安全限制，修复慢速分块图片偶发截断导致二维码漏识别的问题。
- 踢人/禁言 API 现在校验 OneBot 返回状态；协议端拒绝或调用失败时不再误报成功或保留整段处罚冷却，也不会登记无效的定时解禁，后续违规可重新触发处罚。

### 修复：名片审核与可维护性

- 新增 `card_audit_admin_exempt`（默认开启，可按群覆盖）：群主、群管理员、插件及 AstrBot 管理员跳过违规名片审核；名片变更日志仍记录，保护名单的强制还原仍优先执行。
- 周期名片同步直接复用成员列表中的角色，避免逐成员重复查询；普通成员仍按原规则审核。
- 将审核上下文和图片审核从超大的 `moderation.py` 拆分为独立 Mixin，群配置/名片数据从 `storage.py` 拆分为 repository mixin，集中管理排序、缓存、OCR、二维码、下载和资源生命周期，并补充并发、顺序、降级及配置合约回归测试。
...
- 统一 `pyahocorasick` 与纯 Python Trie 降级路径的重叠命中排序，确保安装可选依赖前后返回结果稳定一致。

## v2.7.3 - 2026-07-23

### 修复：多层嵌套消息漏审

- 递归解析 `Node` / `Nodes` / `Forward` / `JSON` / `App`，支持转发中的转发、卡片中的 JSON 字符串和 AstrBot 组件枚举，不再只检查第一层文本。
- `scan_forward_msg=false` 时统一跳过内嵌转发正文；`Reply` 引用内容仍明确排除，避免重复审核他人历史消息。
- 转发发送者昵称不再混入正文，避免昵称里的敏感词误判消息作者；失败的 OneBot API 响应也不会被当作消息内容。
- 加入深度、节点数、字符数、远程请求数、循环引用和单请求 20 秒超时限制，防止恶意深层/超宽消息拖垮审核。
- 递归正文在 5 万字符展示截断前执行最多 10 万字符的完整规则初筛，并保留命中证据；超过字符、深度或节点硬上限时按本地规则处置，堵住前置填充绕过并限制无 AC 环境下的同步耗时。
- 长消息送 LLM 前保留规则命中附近片段；LLM 超时或返回格式异常时，仅高置信辱骂规则由本地兜底处罚，广告泛词和单独命中低置信方言词 `啥子` 时放行，避免 Provider 故障扩大误封。

### 增强：辱骂词库与匹配性能

- 随包辱骂规则扩展至 24,014 条：24,000 条明确辱骂短语/安全分隔变体、13 条 ASCII 边界正则，以及反馈词 `啥子`；同时覆盖 `傻子`、`cs` 等反馈写法。
- `cs` / `cnm` / `cao` / `nmsl` / `wdnmd` / `wcnm` / `shabi` / `sb` / `fuck` / `bitch` / `shit` / `damn` / `asshole` 使用 ASCII 边界正则并支持常见分隔符，避免误伤正常英文子串。
- 移除旧版全字符同音替换自动规则及 13 条可精确识别的空描述 seed 聚合规则，修复 `USB设备`、`Gundam`、`five people`、`shitake` 等误报；仅清理旧自动来源或空描述内置规则，保留带自定义描述的同形规则。
- 未安装 `pyahocorasick` 时使用纯 Python Trie 扫描字面量，不再对 2 万余条规则逐条运行正则；自定义配置词按字面量处理，不会把 `.`、`+` 等字符误当正则。

### 修复：名片监控、入群审核与 API 结果

- 收到标准 `group_increase` 后立即查询新成员名片并审核，不再要求成员先发言；跳过机器人自身入群，且不阻断欢迎类插件继续处理事件。
- 增加名片周期同步兜底（默认 120 秒，可设 30-3600 秒）和 WebUI 配置，弥补部分协议端不发送 `group_card` 的问题。
- 名片查询/同步失败时保留旧快照并重试；修复空列表清空快照、旧轮询覆盖实时事件、还原失败后不再复查等问题。
- 加群通过词默认优先于通用词库/广告初筛，显式拒绝词仍最高优先；新增 `join_accept_overrides_lexicon` 开关控制兼容行为。
- 新增可选的入群 LLM 语义审核与自定义审核标准，默认关闭并支持按群覆盖；显式规则优先，通用词库/广告候选交给 LLM 二审，调用失败时按本地候选或默认动作安全降级。
- OneBot 返回 `status=failed` 或非零 `retcode` 时不再误记为处理成功；群号仅存在原始事件字典时也正确执行群黑白名单检查。
- 消息、OCR、名片、申诉和入群 LLM 调用增加排队/执行超时，群历史及成员信息 API 增加 20 秒超时；不可信消息中的提示词分隔符会先转义，避免伪造审核段落。

## v2.7.2 - 2026-07-17

### 新增：`/回复撤回` 静默开关（Issue #44）

新增配置项 `recall_reply_notice`（默认 `true`，保持原行为）。关闭后 `/回复撤回` 撤回成功时不再回复
「已撤回被引用的消息」——撤回本就是让内容安静消失，再回一句反而多一条噪音。
**撤回失败的错误提示不受此开关影响**，仍会提示操作者（否则用户无法得知撤回未成功，例如超过 2 分钟撤回窗口）。

注：此项与 v2.7.1 修复的 `auto_moderate_notice` 是两个不同的东西——
`auto_moderate_notice` 管的是【自动审核】撤回违规消息后的群内提示，
`recall_reply_notice` 管的是【管理员手动执行 /回复撤回 指令】后的确认回复。
