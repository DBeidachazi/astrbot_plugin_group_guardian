...
- 应用新的复盘候选时追加到现有修正规则，不再覆盖上一轮已应用内容。
- 相同候选（含多段规则）自动去重；修正规则总长度超过 12,000 字符时拒绝应用并保留候选，避免静默丢失旧规则。
- 回滚按“应用前规则 + 当前候选”的完整结果校验，强制从后向前回滚，支持中断恢复并兼容 v2.8.2 已应用的旧候选。
- 限制单条 LLM 修正规则长度并要求输出简洁规则，控制复盘提示词成本。
- 增加多候选追加、重复候选、真实 SQLite 重启回滚和总长度边界回归测试。
