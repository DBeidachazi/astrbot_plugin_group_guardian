...
> 当前版本：**v2.8.2** | 最低 AstrBot 版本：**>=4.24.2**
...
插件管理页始终挂载在 AstrBot 内置 Dashboard，通过 `register_web_api` 复用 Dashboard 路由和登录鉴权；
插件不启动独立 Web 服务或额外监听端口。在 AstrBot 管理面板中即可完成所有操作：

| 标签页 | 功能 |
| ------ | ---- |
| **总览** | 实时统计卡片、今日拦截排行、趋势/分布/时段/群排行图表 |
| **群管理** | 群卡片网格、成员列表、单人/批量禁言/解禁/踢人/设名片/头衔/管理、远程执行群管操作 |
| **违规记录** | 被撤回用户聚合视图、完整审核日志、人工标记误判/确认违规、AI 复盘候选审批与回滚、批量删除、CSV 导出 |
...
- **Base 系列解码审核**：受限识别 Base16/32/58/62/64/Base64URL/85 与双层、跨消息编码，把可读结果送入现有规则、上下文和 LLM 语义审核
- **AI 误判复盘**：管理员在违规记录中标记误判后，由 LLM 生成待确认的审核修正规则；只在人工应用后生效，支持拒绝、冲突保护、回滚和审计
...
WebUI 有**独立的「名片监控」页面**。名片监控总开关**默认关闭**；周期同步兜底
...
让插件从群聊上下文**自动学习本群特有的引流/广告黑话**（如「AI中转」「plus日抛team」），
沉淀进**按群独立的自训练词库**，逐步自我增强审核能力。WebUI 有独立「词库学习」页。

**工作流**：被动采集群消息进按群缓冲 → 周期（默认 300s）把样本交给学习模型 →
LLM 找出反复出现、又不在现有词库里的引流词（带置信度/理由/样例）→ 跨轮累计出现次数 →
存为候选 → 审批生效 → 并入该群匹配器参与初筛与撤回判断。
...
- **多维度自动审批门**（`lexicon_learn_auto_apply`，默认关）：候选词要自动生效（进而自动参与撤回），
  必须**同时通过**五个维度——① 置信度 ② 跨轮出现频次 ③ 词形 ④ 非白名单常用词
  ⑤ **对抗式 LLM 复核**（专门反驳「会不会误伤正常消息」，从严）。任一不过即转人工。
...
| `lexicon_learn_provider_id` | 空 | 学习模型（留空复用审核模型；可单独指定便宜模型省成本） |
| `lexicon_learn_allowlist` | `[]` | 永不学习的词（保护本群常用词/商品名） |

> **RAG 说明**：当前学习核心用可配置的 LLM（`lexicon_learn_provider_id`）从上下文挖掘关键词，
> 自训练数据库即 `learned_keywords` 表（每群独立）。向量嵌入式 RAG 聚类为后续增强方向。
...
| `base_decode_enabled`       | Base16/32/58/62/64/85 等编码解码审核（可按群覆盖） | `true` |
| `moderation_review_enabled` | 定期复盘人工误判样本，只生成候选、不自动应用 | `false` |
| `moderation_review_interval` | 自动误判复盘间隔（秒） | `86400` |
| `moderation_review_min_samples` | 自动复盘最少误判样本数；手动复盘只需一条 | `3` |
| `llm_moderation_review_guidance` | 管理员已应用、可审计回滚的误判修正规则 | 空 |
...
- 每条匹配规则都有独立开关，可按需关闭
- 词库关键词和正则规则可在 WebUI「规则中心」热更新
- 在 WebUI「违规记录」把处罚日志标记为误判，可立即或定期生成修正规则候选；候选经人工确认后才
  参与审核，并保留拒绝、应用、回滚和审计记录
